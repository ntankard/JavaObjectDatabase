...
import static com.ntankard.javaObjectDatabase.CoreObject.Field.DataField.SourceMode.DERIVED;
import static com.ntankard.javaObjectDatabase.CoreObject.Field.DataField.SourceMode.VIRTUAL_DERIVED;
import static com.ntankard.javaObjectDatabase.CoreObject.Field.DataField_Instance.NewFieldState.*;


public class DataField_Instance<FieldType> {
     ...
     */
    private final DataField<FieldType> dataField;
     ...
     * Constructor
     */
    public DataField_Instance(DataField<FieldType> dataField, DataObject container) {
        ...
        this.notifyParentListener = (field, oldValue, newValue) -> {
            if (DataObject.class.isAssignableFrom(field.dataField.getType())) {
                if (field.dataField.isTellParent()) {
    ...
    public void allowValue() {
        ...
        if (dataField.getDataCore_factory() != null) {
            this.dataCore = dataField.getDataCore_factory().createCore(this);
    ...
    public void forceNotify() {
        if (DataObject.class.isAssignableFrom(dataField.getType())) {
            if (dataField.isTellParent()) {
                if (value != null) {
    ...
    public void remove() {
        ...
        if (!dataField.getDependantFields().isEmpty())
    ...
    public void set(FieldType toSet) {
        if (!dataField.getCanEdit() && !state.equals(N_ATTACHED_TO_OBJECT))
            throw new IllegalStateException("This field can only be set once");

        if (dataField.getSourceMode().equals(VIRTUAL_DERIVED)) {
            dataField.getSetterFunction().set(toSet, getContainer());
            return;
        }

        if (dataField.getSourceMode().equals(DERIVED))
     ...
     * @param toSet The value to set
     */
    void setFromDataCore(FieldType toSet) {
        if (dataField.getSourceMode().equals(DataField.SourceMode.DIRECT))
    ...
    public boolean doFilterCheck(FieldType toCheck, FieldType pastValue) {
        for (FieldFilter filter : dataField.getFilters()) {
